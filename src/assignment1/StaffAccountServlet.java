...
@WebServlet("/StaffAccountServlet")
public class StaffAccountServlet extends HttpServlet {
	...
	try {
		...
		try {
			...
			String StaffOptions = req.getParameter("StaffOptions"); 	  
			
			if(StaffOptions.equals("AssignBooks")) 
			{
				res.sendRedirect("/LibrarySystem/jsp/StaffAssignBooks.jsp");   
			}
			
			if(StaffOptions.equals("ExpiryDates"))
			{
				res.sendRedirect("/LibrarySystem/jsp/StaffChangeExpiryDates.jsp");
			}
	
	     }
		catch(Exception e) {
			out.println("There was an error redirecting from jsp to servlet");
		}
